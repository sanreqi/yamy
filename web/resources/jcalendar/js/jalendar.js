...
$(function () {    
    (function ($) {
        $.fn.jalendar = function (options) {
            ...
            $this.find('.submit1').on('click', function(){
                ...
                $.ajax ({
                    ...
                    success: function(responseData) {
                        if (responseData.status == 1) {
                            $this.prepend(div('div', 'added-event').attr({'data-date':thisDay, 'data-time': time, 'data-title': title, 'data-id': dataId}));
                            $this.find('.day.this-month.selected').append(
                                div('div','event-single').attr('data-id', responseData.data_id).append(
                                    div('p','').text(title),
                                    div('div','details').append(
                                        //div('div', 'clock').text(time),
                                        div('div', 'erase')
            ...
            $this.on('click', '.event-single .erase', function(){
                var _this = $(this);
                var id = _this.parents(".event-single").attr("data-id");
                if (id == "0") {
                    alert("不能删除回款信息");
                    return false;
                } else {
                    $.ajax ({
                        url: "/main/delete-remark",
                        type: "post",
                        dataType: "json",
                        data: {
                            id: id
                        ...
                        success: function(responseData) {
                            if (responseData.status == 1) {
                                _this.parents(".event-single").animate({'height': 0}, function(){
                                    $(this).remove();
                                    calcTotalDayAgain();
                                    calcScroll();
                                });
