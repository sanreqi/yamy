...
class Detail extends \yii\db\ActiveRecord {
    ...
    public static function getProfitsByPeriod($startTime, $endTime) {
        ...
        $sql = 'SELECT SUM(current_balance) AS sum FROM p2p_detail AS SUM WHERE id in (SELECT MAX(id) FROM p2p_detail WHERE is_deleted=0 AND time BETWEEN ' . $startTime . ' AND ' . $endTime . ' GROUP BY account_id)';
